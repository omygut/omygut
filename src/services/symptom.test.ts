...
import { addSymptomRecord, getRecentSymptomRecords, deleteSymptomRecord } from "./symptom";
...
describe("symptom service", () => {
  ...
  describe("addSymptomRecord", () => {
    it("should add a symptom record and return id", async () => {
      const id = await addSymptomRecord({
        date: "2026-04-11",
        time: "10:00",
  ...
  describe("getRecentSymptomRecords", () => {
    it("should return empty array when no records", async () => {
      const records = await getRecentSymptomRecords(10);
    ...
    it("should respect limit parameter", async () => {
      ...
      const records = await getRecentSymptomRecords(3);
  ...
  describe("deleteSymptomRecord", () => {
    it("should delete a record", async () => {
      ...
      const beforeRecords = await getRecentSymptomRecords(100);
      ...
      const afterRecords = await getRecentSymptomRecords(100);
      expect(afterRecords.length).toBe(countBefore - 1);
    });

    it("should not throw when deleting non-existent record", async () => {
      await expect(deleteSymptomRecord("non_existent_id")).resolves.not.toThrow();
    });
  });
});
